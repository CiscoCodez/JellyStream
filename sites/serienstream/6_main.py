#!/usr/bin/env python3
"""
Main Orchestrator - SerienStream Database Builder
Runs all scraping scripts in sequence to build complete database
...
class SerienStreamOrchestrator:
    ...
    def setup_logging(self):
        ...
        # Log startup
        self.logger.info("="*60)
        self.logger.info("SerienStream Pipeline Started")
    ...
    def run_full_pipeline(self) -> bool:
        ...
        print("🚀 SerienStream Database Builder")
        print("🎯 Running all scripts in sequence with batch processing")
        if self.limit:
            print(f"📊 Limit: {self.limit} series (testing mode)")
        else:
            print("📊 Limit: None (full database)")
        print(f"📦 Batch size: {self.batch_size} series per batch")
        print("=" * 60)
        ...
        try:
            ...
            if not self.run_script('catalog', self.scripts['catalog'], 'Catalog Scraper (Series Names & URLs)'):
                return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description='SerienStream Database Builder')
    ...
    orchestrator = SerienStreamOrchestrator(limit=args.limit, batch_size=args.batch)
